...
init()
{
	...
	registerScoreInfo( "surv", 1 );
	registerScoreInfo( "knife", 2 );
	registerScoreInfo( "kill", 4 );
	registerScoreInfo( "spec", 5 );
	registerScoreInfo( "seek", 6 );
	registerScoreInfo( "bonus", 10 );
	registerScoreInfo( "hide", 12 );
	registerScoreInfo( "ch", 20 );
	registerScoreInfo( "killpro", 0 );
	registerScoreInfo( "friendknife", 0 );
	registerScoreInfo( "lasthider", 0 );
...
{
	a = getDvarInt("scr_hns_point_" + type);
	if (a)
		level.scoreInfo[type]["value"] = a;
	else
...
// HNS: Yes. GSC can't divide big numbers properly.
divide(a, b)
{
	r = 0;
	m = int(a[0]);
...
encodeStat(s)
{
	...
	s = string(s);
	for (i = 0; i < level.code.size; i++)
	{
		val = getSubStr(level.code[i], 1);
		switch (level.code[i][0])
		{
			case "a": // Addition
				s = string(int(s) + int(val));
			break;
			case "s": // Substraction
				s = string(int(s) - int(val));
			break;
			...
			case "o": // Order
				n = "";
				for (j = 0; j < min(val.size, s.size); j++)
				{
					n += s[int(val[j])];
					resetTimeout();
				}
			...
			case "r": // Replace
				a = int(val[0]);
				b = int(val[1]);
				if (a < s.size && b < s.size && (a || int(s[b])) && (b || int(s[a])))
				{
					n = "";
					for (j = 0; j < s.size; j++)
					{
		...
		switch (level.code[i][0])
		{
			...
			case "o":
				...
				if (n[0] != "0")
				{
					...
					if (val.size < s.size)
						s = r + getSubStr(s, val.size);
					else
						s = r;
				}
			break;
			case "r": // Same as encode
				a = int(val[0]);
				b = int(val[1]);
				if (a < s.size && b < s.size && (a || int(s[b])) && (b || int(s[a])))
				{
...
setRankXP(xp)
{
	// Defense line because of an error with an unknown source
	if (!isDefined(xp)) {
		return;
	}

	self.pers["rankxp"] = xp;
	self setStat(level.stat, encodeStat(xp));

	// We have to calculate these here, because the substraction of menu always go full retard
	self setClientDvars(
		"rankxp", xp,
		"realrankxp", xp - level.defaultXP
	);
	level notify("rankxp", self);
}
...
onPlayerConnect()
{
	while (true)
	{
		...
		// There was an error here, probably it won't fix anything, but give it a try
		if (!isDefined(player))
			continue;
		...
		// HNS
		// Against level hack
		// (2308 = Headshots - Against EasyAccount) - Probably useless, but I won't touch it, since no problems with it. At least everyone will know if someone tried using it.
		// (2314 = Player time must be minimum 10 hours) - DEPRECATED
		// (1678464720 = Level 100) - DEPRECATED
		// (rankxp - 1614807542 + encoding to prevent searching for this value)
		stat = player getStat(level.stat);
		if (!stat)
		{
			player setRankXP(level.defaultXP);
		}
		else
		{
			stat = decodeStat(stat);
			if (stat < level.defaultXP || stat > 1678717056 || player getStat(2308)/* || (stat >= 1678464720 && player.playTime < 36000)*/)
			{
				for (i = 0; i < level.players.size; i++)
				{
					level.players[i] iPrintLn(level.s["tryhack" + level.players[i].lang], player.name);
				}
				player.invalid = true;
				player setRankXP(level.defaultXP);

				player setStat(2308, 0);
			}
			else
			{
				player.pers["rankxp"] = stat;
				player setClientDvars(
					"rankxp", stat,
					"realrankxp", stat - level.defaultXP
				);
			}
			//level notify("rankcheck", player);
		}
		...
		// Certainly it won't work in developer, so don't be mad if ranks are buggy in menus there
...
onPlayerSpawned()
{
	...
	for(;;)
	{
		...
		if(!isdefined(self.hud_rankscroreupdate))
		{
			...
			self.hud_rankscroreupdate.font = "objective";//default
...
giveRankXP( type, value )
{
	...
	if (type != "level" && (!level.playerCount["allies"] || !level.playerCount["axis"]))
		return;
...
updateRank()
{
	...
	self maps\mp\gametypes\_persistence::statSet( "rank", newRankId );
	self maps\mp\gametypes\_persistence::statSet( "minxp", int(level.rankTable[newRankId][2]) );
	self maps\mp\gametypes\_persistence::statSet( "maxxp", int(level.rankTable[newRankId][7]) );
...
updateRankAnnounceHUD()
{
	...
	if (self.pers["rank"] == 115)
		notifyData.sound = "maxlevel";
	else
	...
	*/
	...
	// HNS
	if (subRank == 1 || self.pers["rank"] > 55)
	{
		for (i = 0; i < level.players.size; i++)
		{
			if (level.players[i] != self)
			{
				level.players[i] iPrintLn(&"RANK_PLAYER_WAS_PROMOTED", self, newRankName);
			}
		}
	}
	logPrint("HNS: " + self.name + " is promoted to level " + self.pers["rank"] + "!\n");
...
updateRankScoreHUD( amount )
{
	...
	if( isDefined( self.hud_rankscroreupdate ) )
	{
		...
		else
		{
			...
			self.hud_rankscroreupdate.color = (1,0.75,0.5);
		}
...
{	
	...
	return self getRankForXp(self.pers["rankxp"]);
}
...
incRankXP( amount )
{
	...
	//self setStat(level.stat, encodeStat(self.pers["rankxp"]));
	//self setStat(3175, newXp - 1614807542);

	// HNS
	self setRankXP(newXp);
}

...
init()
{
	...
	// HNS
	precacheItem("hns_high_mp");
	precacheItem("hns_climb_mp");
	precacheItem("hns_door_mp");
...
//	thread maps\mp\_teargrenades::main();
	...
	// HNS: Stolen from _class - Needed for stow
	level.tbl_weaponIDs = [];
	for( i=0; i<150; i++ )
	{
...
onPlayerSpawned()
{
	...
	for(;;)
	{
		...
		//self thread updateStowedWeapon(); // "failed to detach model 'weapon_m40a3' from tag 'tag_stowed_back'" ~ Sometimes, I don't know why
	}
}
...
watchWeaponUsage()
{
	...
	{	
		...
		if (curWeapon == "colt45_silencer_mp") // HNS
			self.hiderAmmo--;
